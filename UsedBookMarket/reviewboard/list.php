...
<?php include "../wrapper.php"  //초기 화면 구성?>
  ...
  <div style="margin:10px auto;padding:40px;width:60em;height:25em">
    <table id="reviewboard">
      ...
      <?php
        ...
        {
          $sql="select * from reviewboard order by good desc ".$sqlLimit;
        }
        else if($sorting=="hit"){
          $sql="select * from reviewboard order by hit desc ".$sqlLimit;
        }
        else if($sorting=="search_title")
        {
          $sql="select * from reviewboard where title like '%".$value."%' order by no desc ".$sqlLimit;
        }
        else if($sorting=="search_content")
        {
          $sql="select * from reviewboard where content like '%".$value."%' order by no desc ".$sqlLimit;
        }
        else if($sorting=="search_nickname")
        {
          $sql="select * from reviewboard where nickname='".$value."' order by no desc ".$sqlLimit;
        }
        else if($sorting=="search_tc")
        {
          $sql="select * from reviewboard where (title like '%".$value."%' or content like '%".$value."%') order by no desc ".$sqlLimit;
        }
        else{}
  ...
  <?php
    if($sorting=="date")
    {
      $sql="select count(*) as cnt from reviewboard order by no desc ";
    }
    else if($sorting=="good")
    {
      $sql="select count(*) as cnt from reviewboard order by good desc ";
    }
    else if($sorting=="hit"){
      $sql="select count(*) as cnt from reviewboard order by hit desc ";
    }
    else if($sorting=="search_title")
    {
      $sql="select count(*) as cnt from reviewboard where title like '%".$value."%' order by no desc ";
    }
    else if($sorting=="search_content")
    {
      $sql="select count(*) as cnt from reviewboard where content like '%".$value."%' order by no desc ";
    }
    else if($sorting=="search_nickname")
    {
      $sql="select count(*) as cnt from reviewboard where nickname='".$value."' order by no desc ";

    }
    else if($sorting=="search_tc")
    {
      $sql="select count(*) as cnt from reviewboard where (title like '%".$value."%' or content like '%".$value."%') order by no desc ";
    }
    else{}
